...
<p align="center" style="font-size:18px;">Practice MCQ</p>
<div class="specer">&nbsp;</div>
<h1 align="center" style="color:#000000;"><strong>Chapter</strong></h1>
...
<tbody>
<?php
$sql="SELECT DISTINCT `chapter`,`subject` FROM `study_exam_question` WHERE `course`='".getCourseDetails($_SESSION['uid'],'course')."' AND `subject`='".getSubjectDetails($_SESSION['uid'],'subject')."' ORDER BY `chapter` ASC";
$res=query($sql);
$num=numrows($res);
if($num>0)
{
...
<tr class="<?=$class?>">
<td align="center"><a href="practice-exam-start.php?chapter=<?=$fetch['chapter']?>" style="color:#000000;"><?=ucwords($fetch['chapter'])?></a></td>
<?php $i++;}} else{?>
